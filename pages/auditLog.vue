<template>
  <div class="space-y-6 animate-fade-in">
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="flex flex-col lg:flex-row items-start lg:items-center gap-4">
          ...
          <div class="relative flex-1 max-w-xs">
            <select
              ...
              @change="auditLogStore.setUserFilter(($event.target as HTMLSelectElement).value)"
            >
              <option v-for="user in auditLogStore.uniqueUsers" :key="user" :value="user">
            ...
            <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none text-[var(--color-text-tertiary)]">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          ...
          <div class="relative flex-1 max-w-xs">
            <select
              ...
              class="input-base pl-10 pr-10 appearance-none cursor-pointer"
              @change="auditLogStore.setActionFilter(($event.target as HTMLSelectElement).value as any)"
            >
        ...
        <div class="flex items-center gap-3">
          <div class="relative flex-1">
            <input
              ...
              @input="auditLogStore.setSearchQuery(($event.target as HTMLInputElement).value)"
            />
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-[var(--color-text-tertiary)]">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div v-if="hasActiveFilters" class="flex flex-wrap gap-2">
          ...
          >
            ...
            <button @click="auditLogStore.setDateRange('all')" class="hover:bg-[var(--color-accent)]/20 rounded-full p-0.5">
          ...
          >
            ...
            <button @click="auditLogStore.setUserFilter('all')" class="hover:bg-[var(--color-accent)]/20 rounded-full p-0.5">
              <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          ...
          >
            ...
            <button @click="auditLogStore.setActionFilter('all')" class="hover:bg-[var(--color-accent)]/20 rounded-full p-0.5">
              <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <div class="card p-4">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-[var(--color-text-secondary)]">Total Events</p>
            <p class="text-2xl font-bold text-[var(--color-text-primary)] mt-1">{{ auditLogStore.counts.total }}</p>
          </div>
      ...
      <div class="card p-4">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-2xl font-bold text-[var(--color-success)] mt-1">{{ auditLogStore.counts.success }}</p>
          </div>
      ...
      <div class="card p-4">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-2xl font-bold text-[var(--color-danger)] mt-1">{{ auditLogStore.counts.failed }}</p>
          </div>
    ...
    <div class="card overflow-hidden">
      <div v-if="auditLogStore.filteredLogs.length === 0" class="p-12 text-center">
        <svg
        ...
        <p class="mt-3 text-sm text-[var(--color-text-secondary)]">No audit logs found</p>
        <p class="text-xs text-[var(--color-text-tertiary)]">Try adjusting your filters</p>
      </div>
      ...
      <div v-else>
          ...
          <table class="w-full">
            <thead>
              <tr class="border-b border-[var(--color-border-primary)]">
                <th class="px-6 py-3.5 text-left text-xs font-semibold text-[var(--color-text-tertiary)] uppercase tracking-wider w-8"></th>
                <th class="px-6 py-3.5 text-left text-xs font-semibold text-[var(--color-text-tertiary)] uppercase tracking-wider">
                  Timestamp
                </th>
                <th class="px-6 py-3.5 text-left text-xs font-semibold text-[var(--color-text-tertiary)] uppercase tracking-wider">
                  User
              ...
              <template v-for="log in auditLogStore.filteredLogs" :key="log.id">
                ...
                >
                  ...
                  <td class="px-6 py-4">
                    ...
                    >
                      <div v-html="getActionIcon(log.action)" class="w-3.5 h-3.5" />
                  ...
                  <td class="px-6 py-4">
                    <span class="text-sm text-[var(--color-text-secondary)] font-mono">
                      {{ log.ipAddress }}
                    </span>
                  </td>
                </tr>
                ...
                <tr v-if="expandedRows.has(log.id)" class="bg-[var(--color-bg-secondary)]">
                  <td colspan="7" class="px-6 py-4">
                    <div class="pl-12 space-y-3">
                      <div>
                        <p class="text-xs font-semibold text-[var(--color-text-tertiary)] uppercase tracking-wider mb-1">Details</p>
                        <p class="text-sm text-[var(--color-text-primary)]">{{ log.details }}</p>
                      </div>

                      <div v-if="log.metadata && Object.keys(log.metadata).length > 0">
                        <p class="text-xs font-semibold text-[var(--color-text-tertiary)] uppercase tracking-wider mb-2">Metadata</p>
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                          ...
                          >
                            <p class="text-xs text-[var(--color-text-tertiary)] capitalize mb-1">{{ formatKey(key as string) }}</p>
                            <p class="text-sm font-medium text-[var(--color-text-primary)]">{{ formatValue(value) }}</p>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <div class="md:hidden divide-y divide-[var(--color-border-secondary)]">
          <div
            v-for="log in auditLogStore.filteredLogs"
            :key="log.id"
          ...
          >
            ...
            <div class="flex items-start justify-between gap-3">
              <div class="flex items-center gap-2">
                ...
                <div>
                  <p class="text-sm font-medium text-[var(--color-text-primary)]">{{ log.user }}</p>
                  <p class="text-xs text-[var(--color-text-tertiary)]">{{ formatTimestamp(log.timestamp) }}</p>
                </div>
              </div>
              <svg
                :class="[
                  'w-4 h-4 text-[var(--color-text-tertiary)] transition-transform flex-shrink-0',
                  expandedRows.has(log.id) ? 'rotate-90' : ''
                ]"
                fill="none"
                viewBox="0 0 24 24"
            ...
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <span class="text-xs text-[var(--color-text-secondary)]">{{ log.resource }}</span>
            </div>
            ...
            <div class="flex items-center justify-between text-xs">
              ...
              <span class="text-[var(--color-text-tertiary)] font-mono">{{ log.ipAddress }}</span>
            </div>

            <!-- Expanded Details -->
            <div v-if="expandedRows.has(log.id)" class="pt-3 border-t border-[var(--color-border-secondary)] space-y-3">
              <div>
                <p class="text-xs font-semibold text-[var(--color-text-tertiary)] uppercase tracking-wider mb-1">Details</p>
                <p class="text-sm text-[var(--color-text-primary)]">{{ log.details }}</p>
              </div>
              ...
              <div v-if="log.metadata && Object.keys(log.metadata).length > 0">
                ...
                <div class="grid grid-cols-2 gap-2">
                  <div
                    v-for="(value, key) in log.metadata"
                    :key="key"
                    class="bg-[var(--color-bg-secondary)] rounded-lg p-2"
                  >
                    <p class="text-xs text-[var(--color-text-tertiary)] capitalize mb-0.5">{{ formatKey(key as string) }}</p>
                    <p class="text-xs font-medium text-[var(--color-text-primary)]">{{ formatValue(value) }}</p>
                  </div>
                </div>
              </div>
            </div>
...
function getActionIcon(action: AuditActionType): string {
  const icons: Record<AuditActionType, string> = {
    login: '<svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" /></svg>',
    create: '<svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" /></svg>',
    update: '<svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>',
    ...
    export: '<svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>',
...
function getActionClass(action: AuditActionType): string {
  const classes: Record<AuditActionType, string> = {
    ...
    update: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/20 dark:text-yellow-400',
