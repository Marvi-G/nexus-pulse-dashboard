<template>
  <div class="card overflow-hidden">
            ...
            >
              <div class="flex items-center gap-1.5">
                ...
                <svg
                  ...
                  :class="['w-3.5 h-3.5 transition-transform', sortDir === 'desc' ? 'rotate-180' : '']"
          ...
          >
            <td class="px-6 py-4">
              <div class="flex items-center gap-3">
                ...
                <span class="text-sm font-medium text-[var(--color-text-primary)]">{{ row.name }}</span>
              </div>
            </td>
            <td class="px-6 py-4 text-sm text-[var(--color-text-secondary)]">{{ row.email }}</td>
            <td class="px-6 py-4">
            ...
            <td class="px-6 py-4">
              <UiBadge
                :label="row.status"
                :variant="row.status === 'Active' ? 'active' : row.status === 'Pending' ? 'pending' : 'inactive'"
              />
            </td>
            <td class="px-6 py-4 text-sm text-[var(--color-text-secondary)]">{{ formatDate(row.joinDate) }}</td>
            <td class="px-6 py-4">
    ...
    <div class="md:hidden divide-y divide-[var(--color-border-secondary)]">
      <div v-for="row in rows" :key="row.id" class="p-4 space-y-3">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-[var(--color-accent)] to-purple-500 flex items-center justify-center text-white text-sm font-semibold">
              {{ getInitials(row.name) }}
            </div>
            <div>
              <p class="text-sm font-medium text-[var(--color-text-primary)]">{{ row.name }}</p>
              <p class="text-xs text-[var(--color-text-tertiary)]">{{ row.email }}</p>
            </div>
          </div>
        </div>
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <UiBadge
            ...
            :variant="row.status === 'Active' ? 'active' : row.status === 'Pending' ? 'pending' : 'inactive'"
          />
          <span class="text-xs text-[var(--color-text-tertiary)]">{{ formatDate(row.joinDate) }}</span>
        </div>
        <div class="flex items-center gap-2 pt-1">
          <UiButton size="sm" variant="secondary" @click="$emit('edit', row)">Edit</UiButton>
          <UiButton size="sm" variant="danger" @click="$emit('delete', row)">Delete</UiButton>
        </div>
      </div>
    </div>
    ...
    <div v-if="rows.length === 0" class="p-12 text-center">
      <svg class="w-12 h-12 mx-auto text-[var(--color-text-tertiary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
        <path stroke-linecap="round" stroke-linejoin="round" d="M15.182 16.318A4.486 4.486 0 0012.016 15a4.486 4.486 0 00-3.198 1.318M21 12a9 9 0 11-18 0 9 9 0 0118 0zM9.75 9.75c0 .414-.168.75-.375.75S9 10.164 9 9.75 9.168 9 9.375 9s.375.336.375.75zm-.375 0h.008v.015h-.008V9.75zm5.625 0c0 .414-.168.75-.375.75s-.375-.336-.375-.75.168-.75.375-.75.375.336.375.75zm-.375 0h.008v.015h-.008V9.75z" />
      </svg>
      <p class="mt-3 text-sm text-[var(--color-text-secondary)]">No users found</p>
      <p class="text-xs text-[var(--color-text-tertiary)]">Try adjusting your search query</p>
    </div>

    <!-- Pagination footer -->
    <div v-if="rows.length > 0" class="px-4 sm:px-6 py-4 border-t border-[var(--color-border-primary)]">
