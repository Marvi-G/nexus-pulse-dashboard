<template>
  ...
  >
    <!-- Logo -->
    <div
      class="flex items-center gap-3 px-4 h-16 border-b border-[var(--color-border-primary)]"
    >
      <div
      ...
      <span
        v-if="!collapsed"
        class="text-lg font-semibold text-[var(--color-text-primary)] tracking-tight transition-opacity duration-200"
        >Pulse</span
      >
      ...
      <button
        v-if="!isOpen"
        class="ml-auto hidden lg:flex items-center justify-center w-8 h-8 rounded-lg hover:bg-[var(--color-bg-hover)] transition-colors"
    ...
    <nav class="flex-1 px-3 py-4 space-y-1">
      ...
      >
        <component
          :is="
            () =>
              h('div', { class: 'w-5 h-5 flex-shrink-0', innerHTML: item.icon })
          "
        />
        ...
        <div
          v-if="collapsed"
          class="absolute left-full ml-2 px-2 py-1 bg-[var(--color-bg-primary)] border border-[var(--color-border-primary)] rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 pointer-events-none whitespace-nowrap z-50"
        >
    ...
    <!-- Bottom section -->
    <div
      class="px-3 py-4 border-t border-[var(--color-border-primary)] space-y-1"
    >
      <!-- Theme toggle -->
      <button
      ...
      >
        ...
        <div
          v-if="collapsed"
          class="absolute left-full ml-2 px-2 py-1 bg-[var(--color-bg-primary)] border border-[var(--color-border-primary)] rounded-lg shadow-lg opacity-0 invisible hover:opacity-100 hover:visible transition-all duration-200 pointer-events-none whitespace-nowrap z-50"
        >
          <span class="text-sm font-medium text-[var(--color-text-primary)]">{{
            isDark ? "Light Mode" : "Dark Mode"
          }}</span>
