<template>
  <aside
    :class="[
      'fixed inset-y-0 left-0 z-40 flex flex-col w-64 bg-[var(--color-bg-sidebar)] border-r border-[var(--color-border-primary)] transition-transform duration-300 lg:translate-x-0',
      isOpen ? 'translate-x-0' : '-translate-x-full',
    ]"
  >
    <!-- Logo -->
    <div class="flex items-center gap-3 px-6 h-16 border-b border-[var(--color-border-primary)]">
      <div class="flex items-center justify-center w-8 h-8 rounded-lg bg-[var(--color-accent)]">
      ...
      <span class="text-lg font-semibold text-[var(--color-text-primary)] tracking-tight">Pulse</span>
    </div>
    ...
    <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
      <NuxtLink
        ...
        @click="closeMobile"
      >
        <component :is="() => h('div', { class: 'w-5 h-5', innerHTML: item.icon })" />
        <span>{{ item.label }}</span>
      </NuxtLink>
    </nav>

    <!-- Bottom section -->
    <div class="px-3 py-4 border-t border-[var(--color-border-primary)] space-y-1">
      <!-- Theme toggle -->
      <button class="sidebar-link w-full" @click="toggleTheme">
        <div class="w-5 h-5" v-html="isDark ? sunIcon : moonIcon" />
...
const sunIcon = '<svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/></svg>';
const moonIcon = '<svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/></svg>';
